...
class AugmentColor(object):
    def __init__(self):
        ...
        self.EV = np.array(
            [1.65513492, 0.48450358, 0.1565086], dtype=np.float32)
        self.sigma = 0.1
        self.color_vec = None
...
class SquarePadding(object):
    ...
    def __call__(self, image, pts=None):
        ...
        expand_image = np.zeros(
            (padding_size, padding_size, 3), dtype=image.dtype)
...
class RandomResizedCrop(object):
    def __init__(self, height, width, scale=(0.3, 1.0), ratio=(3. / 4., 4. / 3.)):
